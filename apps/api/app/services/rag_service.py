...
async def index_document(
    ...
    sql_insert = """
        INSERT INTO content_embeddings (tenant_id, content_id, chunk_index, chunk_text, embedding, metadata)
        VALUES (:tenant_id, :content_id, :chunk_index, :chunk_text, :embedding, :metadata);
    """
...
async def delete_document_index(tenant_id: str, content_id: str, db_session) -> None:
    ...
    sql_delete = """
        ...
        WHERE tenant_id = :tenant_id AND content_id = :content_id;
    """
    from sqlalchemy import text as sa_text
    try:
        await db_session.execute(
...
async def answer_from_knowledge(
    ...
    # 4. Call LLM (using client if provided, otherwise fallback / mocked)
    # The system prompt instructions ensure groundedness
    system_prompt = (
    ...
    if llm_client:
        try:
            # We assume a standard chat completion API interface
            response = await llm_client.chat.completions.create(
                model="gpt-4-turbo",  # or whichever model is wired in
                messages=[

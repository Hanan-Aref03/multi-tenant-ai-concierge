...
_SAFE_REFUSAL = "I can't help with that request. I can still help with product, service, or support questions."
_LEAD_KEYWORDS = (
    "pricing",
    "buy",
    "interested",
...
async def _load_history(redis_client, tenant_id: str, conversation_id: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("Failed to load widget chat history: tenant=%s session=%s error=%s",
...
async def _save_history(
    ...
    try:
        ...
        logger.warning("Failed to save widget chat history: tenant=%s session=%s error=%s",
                       tenant_id, conversation_id, exc)
...
async def _guardrails_check(tenant_id: str, message: str) -> Dict[str, Any]:
    ...
    if not settings.guardrails_url:
        return {"allowed": True, "redacted_message": message, "decision": "allow", "reason": "disabled"}
...
async def chat(
    ...
    try:
        ...
        logger.error("Guardrails check failed: tenant=%s session=%s error=%s", tenant_id, conversation_id, exc)
        raise HTTPException(
    ...
    try:
        ...
        logger.error("Chat pipeline failed: tenant=%s session=%s widget=%s error=%s",
                     tenant_id, conversation_id, claims.widget_id, exc)
        raise HTTPException(
